...
win = tk.Tk()
win.title('Login Here')
win.geometry('350x350+500+200')


name_label = ttk.Label(win, text='User name',font="20").place(x=60,y=10)
name_var = tk.StringVar()
name_entrybox = ttk.Entry(win, width=16, textvariable = name_var).place(x=80,y=40)


password_label = ttk.Label(win, text='Password',font="20").place(x=60,y=80)
...
def second_window():
    ...
    name2_label = ttk.Label(win_2, text='Student name : ', font='10').place(x=10,y=50)
    name2_var = tk.StringVar()
    name2_entrybox = ttk.Entry(win_2, width=60, textvariable = name2_var).place(x=170,y=55)
    branch_label = ttk.Label(win_2, text='Student branch : ',font="20").place(x=10,y=100)
    branch_var = tk.StringVar()
    branch_entrybox = ttk.Entry(win_2, width=60, textvariable = branch_var).place(x=180,y=105)
    Id_label = ttk.Label(win_2, text='Student Regd.id : ',font="20").place(x=10,y=150)
    Id_var = tk.StringVar()
    ...
    def insert():
        ...
        if(name3 == "" or branch == "" or Id == ""):
            m_box.showinfo('Error','Input all details...')
        else: 
            win_3 = tk.Toplevel()
            win_3.title('Subjects')
            win_3.geometry('600x400+500+100')
        
            subjects_label = ttk.Label(win_3, text='Select your subjects',font="20").place(x=60,y=10)
        ...
        def insert2():
            ...
            def insert3():
                ...
                conn = mysql.connect(host='localhost',username='root',password='Abhishek@98',database='students_login')
                cursor = conn.cursor()
                cursor.execute("INSERT INTO abhishek VALUE ('%s','%s','%s','%s','%s','%s')" % (name3,branch,Id,a, b, c))
                ...
                maths = sub3_var.get()
                cgpa = ((physics+chemistry+maths)/3)/10
    ...
    submit_button2 = tk.Button(win_2, text='Submit',bg='grey',command=insert).place(x=210,y=230)
    return True
...
def open_second_window():
    if(name_var.get() == 'abhishek' and pass_var.get() == '123456'):
        second_window() 
